    ...
    .done(function (data) {
      ...
      if (hasItems) {
        ...
        items.forEach(function (item, index) {
          ...
          if (item.thumb_url) {
            var image = $('<div>').css('background-photos', 'url("' + item.thumb_url + '?timestamp=' + item.time + '")');
          } else {
...
function preview(items) {
  ...
  items.forEach(function (item, index) {
    ...
    if (item.thumb_url) {
      carouselItem.find('.carousel-photos').css('background-photos', 'url(\'' + item.url + '?timestamp=' + item.time + '\')');
    } else {
      carouselItem.find('.carousel-photos').css('width', '50vh').append($('<div>').addClass('mime-icon ico-' + item.icon));
    }
